...
import { getOrderStatusLabel } from "@/lib/i18n";
...
export default function OrdersPage() {
  ...
  async function handleMarkCompleted(orderId: number) {
    try {
      ...
      showToast(
        err instanceof Error ? err.message : messages.unableToUpdateOrder,
        "error"
      );
    }
  }
  ...
  if (loading) {
    return <p className="text-gray-500">{messages.loadingOrders}</p>;
  }

  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">{messages.myOrders}</h1>
      {orders.length === 0 ? (
        <p className="text-gray-500">{messages.noOrdersYet}</p>
      ) : (
